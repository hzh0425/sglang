...
class UnifiedRadixCache(BasePrefixCache):
    ...
    def _ensure_node_hash_value(self, node: UnifiedTreeNode) -> None:
        if node is self.root_node or node.hash_value is not None:
            return
    ...
    def _split_node(
        ...
        if self.enable_storage and child.hash_value is None:
            self._ensure_node_hash_value(child)
    ...
    def _add_new_node(
        ...
        if self.enable_storage:
            self._ensure_node_hash_value(parent)
    ...
    def _insert_helper_host(
        ...
        self._touch_node(node)
        if total_len == 0:
            return InsertResult(prefix_len=0, mamba_exist=True, total_len=0)
    ...
    def _free_transfer_host_indices(
        self, transfers: Optional[list[PoolTransfer]]
    ) -> None:
        if not transfers or self.host_pool_group is None:
            return
        for transfer in transfers:
            entry = self.host_pool_group.entry_map.get(transfer.name)
            if (
                entry is None
                or entry.share_indices_with_anchor
    ...
    def prefetch_from_storage(
        ...
        host_lock_params = self.inc_host_lock_ref(last_host_node).to_dec_params()
