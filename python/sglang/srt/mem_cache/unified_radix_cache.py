...
class UnifiedRadixCache(BasePrefixCache):
    ...
    def _reset_l1_only(self) -> None:
        ...
        but keeps its host_value so that subsequent match_prefix can still
        ...
        # 2. Walk tree: release all device resources, keep host_value
        ...
        # 4. Rebuild host leaf sets (all non-root backuped nodes are now evictable host leaves)
        self.evictable_host_leaves = set()
        self._rebuild_host_leaf_sets()

        # 5. Reset component-specific device state
        full_comp = self.components.get(BASE_COMPONENT_TYPE)
        if full_comp is not None:
            full_comp.evictable_device_leaves.clear()
    ...
    def cache_unfinished_req(self, req: Req, chunked=False) -> None:
        ...
        if new_prefix_len > len(new_indices):
            logger.warning(
                "cache_unfinished_req: new_prefix_len=%d > len(new_indices)=%d "
                "(evicted nodes in tree path?)",
    ...
    def _match_post_processor(
        ...
        while last_device_node is not self.root_node and last_device_node.evicted:
            if last_device_node.host_value is not None:
    ...
    def _split_node(
        ...
        # HiCache: update evictable leaf sets for both new parent and child
    ...
    def _add_new_node(
        ...
        # HiCache: register the new leaf as evictable and update parent status
        self._update_evictable_leaf_sets(new_node)
    ...
    def _unevict_node_on_insert(
        ...
        during insert.  Mirrors ``_unevict_node`` in HiMambaRadixCache."""
    ...
    def _insert_helper(
        ...
        while len(key) > 0 and child_key in node.children:
            ...
            if node.evicted:
                # HiCache: unevict node using fresh KV indices from the
                # insert value.  Do NOT count in total_prefix_length —
                # match_prefix also skips evicted nodes' device values.
    ...
    def _evict_device_leaf(self, node: UnifiedTreeNode) -> int:
        ...
        Returns the number of Full KV tokens freed."""
        ...
        if not node.backuped:
            if (
                ...
                # _cascade_evict skips the trigger component, so we must evict
                # Full explicitly — matching the L1-only pattern of
                # _evict_component_and_detach_lru + _cascade_evict.
