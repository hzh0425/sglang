...
class DeepSeekV4CompressedComponent(TreeComponent):
    ...
    def _has_transfer_indices(
        xfer: Optional[PoolTransfer], field: str
    ) -> bool:
        """Check if a peer transfer has non-empty indices for the given field."""
        if xfer is None:
            return False
    ...
    def build_hicache_transfers(
        ...
        # Decide derived-pool transfers based on whether FULL / SWA peers
        # actually have indices, rather than inspecting node data directly.
        ...
        if phase == CacheTransferPhase.BACKUP_HOST:
            # FULL-derived pools: check FULL has device_indices to backup
            if self._has_transfer_indices(kv_xfer, "device_indices"):
                transfers.extend(self._available_full_derived_transfers())
            # SWA-derived state pools: check SWA has device_indices
        ...
        if phase == CacheTransferPhase.LOAD_BACK:
            # FULL-derived pools: check FULL has host_indices to load back
            if self._has_transfer_indices(kv_xfer, "host_indices"):
                transfers.extend(self._available_full_derived_transfers())
            # SWA-derived state pools: check SWA has host_indices
