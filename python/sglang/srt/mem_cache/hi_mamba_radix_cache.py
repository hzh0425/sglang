...
class HiMambaRadixCache(MambaRadixCache):
    ...
    def write_backup(self, node: TreeNode, write_back=False):
        if not self._ensure_node_mamba_host_slot(node):
            return 0
        auxiliary_transfers = self._build_mamba_write_aux(node)
        ...
        if host_indices is not None:
            ...
            node.host_auxiliary_transfers = self._build_mamba_storage_aux(
                node.host_value, node.mamba_host_value
            )
    ...
    def load_back(
        ...
        full_device_indices = self.cache_controller.load(
            ...
            auxiliary_transfers=self._build_mamba_load_aux(
    ...
    def evict_mamba(self, mamba_num: int) -> int:
        ...
        while mamba_num_evicted < mamba_num and self.mamba_lru_list.in_list(x):
            ...
            if x.evicted:
                if not self._ensure_node_mamba_host_slot(x):
                    break
                self.req_to_token_pool.mamba_pool.free(x.mamba_value)
    ...
    def prefetch_from_storage(
        ...
        if prefetch_length > 0 and mamba_host_index is None:
            ...
            self._release_host_node(last_host_node)
            return
        auxiliary_transfers = self._build_mamba_prefetch_aux(
            prefetch_length, mamba_host_index
        )
        if auxiliary_transfers is not None:
            logger.info(
    ...
    def check_prefetch_progress(self, req_id: str) -> bool:
        ...
        matched_length = self._insert_helper_host(
            ...
            mamba_host_indices,
            operation.auxiliary_load_result,
        )

        self.cache_controller.mem_pool_host.free(host_indices[:matched_length])
        completed_pages = min_completed_tokens // self.page_size
        last_page_has_mamba = (
            operation.auxiliary_load_result.hit_page_count_by_name.get("mamba", 0)
            >= completed_pages
        )
    ...
    def _insert_helper_host(
        ...
        has_mamba = auxiliary_load_result is None or (
            auxiliary_load_result.hit_page_count_by_name.get("mamba", 0)
            >= len(hash_value)
        )
        while len(key) > 0 and child_key in node.children.keys():
            node = node.children[child_key]
        ...
        if final_mamba_node is not None and mamba_host_value is not None and has_mamba:
            ...
            final_mamba_node.host_auxiliary_transfers = self._build_mamba_storage_aux(
                final_mamba_node.host_value,
                final_mamba_node.mamba_host_value,
            )
    ...
    def _ensure_node_mamba_host_slot(self, node: TreeNode) -> bool:
    ...
    def _build_mamba_write_aux(
        self, node: TreeNode
    ) -> Optional[list[AuxiliaryTransfer]]:
        if node.mamba_host_value is None or node.mamba_value is None:
            return None
        return [
            AuxiliaryTransfer(
                name="mamba",
    ...
    def _build_mamba_storage_aux(
        self, host_value: torch.Tensor, mamba_host_value: Optional[torch.Tensor]
    ) -> Optional[list[AuxiliaryTransfer]]:
        last_page_index = self._last_page_index(len(host_value))
        if mamba_host_value is None or last_page_index is None:
            return None
        return [
            AuxiliaryTransfer(
                name="mamba",
                ...
                binding=AuxiliaryPageBinding(
                    ...
                    trailing_pages=1,
                    page_indices=[last_page_index],
                ),
            )
        ]

    def _build_mamba_prefetch_aux(
        self, prefetch_length: int, mamba_host_index: Optional[torch.Tensor]
    ) -> Optional[list[AuxiliaryTransfer]]:
        last_page_index = self._last_page_index(prefetch_length)
        if mamba_host_index is None or last_page_index is None:
            return None
        return [
            AuxiliaryTransfer(
                name="mamba",
    ...
    def _build_mamba_load_aux(
        ...
        for load_node in nodes_to_load:
            ...
            if not need_host_load:
                continue
            if load_node.mamba_value is None:
                load_node.mamba_value = self._alloc_with_evict(
                    self.req_to_token_pool.mamba_pool,
                    len(load_node.mamba_host_value),
                    self.evict_mamba,
                    lock_node=load_node,
                    error_message=(
                        f"Can not alloc mamba cache for node {load_node.id}: "
                        f"host={len(load_node.mamba_host_value)}"
                    ),
                )
            ...
            if req is not None and load_node is last_hit_node:
                ...
                mamba_host_indices.append(load_node.mamba_host_value)
                mamba_device_indices.append(req.mamba_pool_idx.unsqueeze(0))
