...
class HybridCacheController(BaseHiCacheController):
    ...
    def _init_extra_host_mem_release_queues(self) -> None:
        ...
        anchor_name = getattr(anchor_entry, "name", None)
        for entry in entries:
            if entry.name == anchor_name or entry.share_indices_with_anchor:
                continue
    ...
    def append_host_mem_release(
        ...
        for transfer in extra_pools or []:
            ...
            if entry is None or entry.share_indices_with_anchor:
                continue
