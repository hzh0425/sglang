...
class MooncakeKVManager(CommonKVManager):
    ...
    def send_kvcache_hisparse(
        ...
        Converts prefill per-page kv_item_lens to per-token, then reuses
        _send_kvcache_generic for token-level RDMA transfer.
        Both prefill MLA buffer and decode host buffer are token-level flat,
                ...
                for req in reqs_to_be_processed:
                    if not req.is_dummy:
                        ...
                        if dst_kv_item_len < src_kv_item_len:
                            # HiSparse: decode per-token < prefill per-page
