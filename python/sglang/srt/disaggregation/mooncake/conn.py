...
class MooncakeKVManager(CommonKVManager):
    ...
    def send_kvcache_hisparse(
        ...
        """HiSparse transfer: P端 page_size > D端 host page_size=1.

        Converts P端 per-page kv_item_lens to per-token, then reuses
        _send_kvcache_generic for token-level RDMA transfer.
        Both P端 MLA buffer and D端 host buffer are token-level flat,
                ...
                for req in reqs_to_be_processed:
                    if not req.is_dummy:
                        ...
                        if dst_kv_item_len < src_kv_item_len:
                            # HiSparse: D端 per-token < P端 per-page
