...
class DecodeTransferQueue:
    ...
    def _next_hicache_load_consumer_index(self) -> int:
        counter = self.tree_cache.cache_controller.layer_done_counter
        return (counter.producer_index + 1) % counter.num_counters

    def _has_available_hicache_load_slot(self) -> bool:
        if not hasattr(self.tree_cache, "is_load_back_event_done"):
            return False
        return self.tree_cache.is_load_back_event_done(
            self._next_hicache_load_consumer_index()
        )

    def _poll_hicache_local_restore(self, decode_req: DecodeRequest) -> None:
    ...
    def _prepare_hicache_local_restore(self, decode_req: DecodeRequest) -> bool:
        """Queue one request's L2->L1 load_back op if its prefix is ready."""
        if decode_req.hicache_restore_status != HiCacheRestoreResult.PENDING:
            return False
        ...
        if (
            len(rematch.device_indices) + len(new_indices)
    ...
    def _process_hicache_local_restores(
        ...
        for decode_req in decode_reqs:
            self._poll_hicache_local_restore(decode_req)

        if not hasattr(self.tree_cache, "is_load_back_event_done"):
            return

        if not self._has_available_hicache_load_slot():
            return

        prepared_reqs: List[DecodeRequest] = []
        for decode_req in decode_reqs:
            if self._prepare_hicache_local_restore(decode_req):
                prepared_reqs.append(decode_req)
        ...
        if consumer_index < 0:
            for decode_req in prepared_reqs:
                decode_req.hicache_restore_status = HiCacheRestoreResult.READY
            return
