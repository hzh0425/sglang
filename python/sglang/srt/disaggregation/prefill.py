...
class PrefillBootstrapQueue:
    ...
    def pop_bootstrapped(
        ...
        for i, (req, poll) in enumerate(zip(self.queue, polls)):
            ...
            # Detect hisparse: D端 per-token item_len < P端 per-page item_len
            hisparse_transfer = False
            kv_mgr = req.disagg_kv_sender.kv_mgr
            if hasattr(kv_mgr, "decode_kv_args_table") and hasattr(
...
class SchedulerDisaggregationPrefillMixin:
    ...
    def send_kv_chunk(
        ...
        if hisparse_transfer:
            # HiSparse: send token-level indices (D端 host pool page_size=1)
