...
def flush_cache(base_url, retries: int = 10, retry_interval: float = 1.0) -> bool:
    """Flush device cache to force remote storage access.

    Retries are needed because the scheduler may still be finishing its last
    decode step (running_batch not yet empty) when the final HTTP 200 OK for
    the last generate request arrives at the client.
    """
    import time
