...
impl StickySessionPolicy {
    pub fn new() -> Self {
        Self::with_ttl_and_reap_interval(DEFAULT_SESSION_TTL, DEFAULT_REAP_INTERVAL)
    }
...
mod tests {
    ...
    fn refreshes_ttl_on_hit() {
        ...
        assert!(sticky.select_existing(&[Arc::clone(&w0)], &ctx).is_some());
        std::thread::sleep(Duration::from_millis(30));
        assert!(
            sticky.select_existing(&[w0], &ctx).is_some(),
