...
"""Real-GPU acceptance coverage for ``policy = "sticky_session_load_based"``."""
...
def _chat(
    ...
    *,
    ...
    return httpx.post(
        ...
        headers={"x-sglang-routing-key": routing_key},
        json={
            "model": model_id,
...
def _wait_for_worker_load(router_url: str, worker_url: str) -> None:
    deadline = time.time() + 60.0
    while time.time() < deadline:
        if _active_prefill_loads(router_url).get(worker_url, 0) > 0:
            return
        time.sleep(0.2)
    raise AssertionError(
        f"worker {worker_url} did not become busy; "
        f"last loads={_active_prefill_loads(router_url)}"
    )


@pytest.mark.real_gpu
@pytest.mark.slow
def test_sticky_session_overrides_load_based_when_worker_is_busy(
    ...
    try:
        with (
            ...
            assigned_worker = _single_success_delta(before, _success_counts(router.base_url))
            ...
            def long_request() -> None:
                try:
                    resp = _chat(
                        router.base_url,
                        spec["model"],
                        "Write a long numbered list for sticky routing validation.",
                        routing_key="session-alpha",
                        max_tokens=1024,
                    )
                    assert resp.status_code == 200, resp.text
            ...
            thread.start()
            _wait_for_worker_load(router.base_url, assigned_worker)
            ...
            assert resp.status_code == 200, resp.text
            same_session_worker = _single_success_delta(
                before, _success_counts(router.base_url)
            )
            assert same_session_worker == assigned_worker

            before = _success_counts(router.base_url)
            resp = _chat(
                router.base_url,
                spec["model"],
                "Use load-based fallback.",
                routing_key="session-beta",
                max_tokens=8,
            )
            assert resp.status_code == 200, resp.text
            other_session_worker = _single_success_delta(
                before, _success_counts(router.base_url)
            )
            assert other_session_worker != assigned_worker
