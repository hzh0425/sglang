...
def _chat(router_url: str, model_id: str, prompt: str, max_tokens: int) -> httpx.Response:
    return httpx.post(
...
def _wait_for_busy_worker(router_url: str, worker_urls: list[str]) -> str:
    ...
    while time.time() < deadline:
        ...
        if busy:
            return busy[0]
        time.sleep(0.2)
    raise AssertionError(f"no worker became busy; last loads={_active_prefill_loads(router_url)}")
...
def test_load_based_routes_to_the_cooler_worker(
    ...
    try:
        with (
            ...
            assert resp.status_code == 200, resp.text
            routed_worker = _single_success_delta(before, _success_counts(router.base_url))
